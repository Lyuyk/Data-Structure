...
public:
    Node();
    bool addStuInfoAtIndex(int index,QString Nm,long long Id,int Ag,QString Gd,QString Ad,QString Br,long long tl,QString BC);//将信息插入到下标
    void inputStuInfo();//输入学生信息
    void deleteStuInfo(long long ID);//删除学生信息
    void outputStuInfo();//输出学生信息
    void sortStuInfoByID();//学生信息排序
    void modifyStuInfo(QString Nm,long long Id,int Ag,QString Gd,QString Ad,QString Br,long long tl,QString BC);//修改学生信息
    bool searchStuInfo(QString &Nm,long long Id,int &Ag,QString &Gd,QString &Ad,QString &Br,long long &tl,QString &BC);//查找学生信息
    int stuInfoIndex();//查询链表长度
...
template <class T>
bool Node<T>::addStuInfoAtIndex(int index, QString Nm, long long Id, int Ag, QString Gd, QString Ad, QString Br, long long tl, QString BC)
{
    ...
    Node<T> *nowNode=head;//创建新的虚拟节点指针
    ...
    Node<T> *addNode=new Node;//创建被插入节点
...
void Node<T>::inputStuInfo()
{
    ...
    head=new Node<T>;//创建新头节点
    if(head==0)
    {
        QMessageBox::about(0,"错误","动态内存分配失败！");
        return;
    }
    Node<T>* tail=head;//创建一个指针复制头部
    ...
    while(!infile.atEnd())
    {
      Node<T>* newNode=new Node;//创建一个新的被插入节点
      if(newNode==0)
      {
          QMessageBox::about(0,"错误","动态内存分配失败");
          return;
...
template <class T>
void Node<T>::deleteStuInfo(long long ID)
{
    Node<T> *point,*before,*bp;
    point=head->next;
...
void Node<T>::sortStuInfoByID()
{
    ...
    Node<T> *pNowNode=new Node;
    pNowNode->next=head;//复制头结点的next域（通过前面判断的都是非空表）
    Node<T> *sortedTail=pNowNode;//指向排好序的链表的末尾

    while(sortedTail->next!=0)
    {
        Node<T> *minNode=sortedTail->next;
        Node<T> *p=sortedTail->next->next;
...
template <class T>
void Node<T>::modifyStuInfo(QString Nm, long long Id, int Ag, QString Gd, QString Ad, QString Br, long long tl, QString BC)
{
    Node<T> *nowNode=head->next;
    bool flag=false;
...
template <class T>
bool Node<T>::searchStuInfo(QString &Nm, long long Id, int &Ag, QString &Gd, QString &Ad, QString &Br, long long &tl, QString &BC)
{
    Node<T> *p=head->next;
    bool flag=false;
